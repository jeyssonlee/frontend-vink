...
import { usePermisos } from "@/hooks/usePermisos"; // ← NUEVO
...
export default function GestionCobranzasPage() {
    ...
    const { tienePermiso } = usePermisos(); // ← NUEVO

    // ── Permisos específicos de esta página ──────────────────────────────────
    const puedeAprobar = tienePermiso("aprobar_cobranzas");
    const puedeRegistrarManual = tienePermiso("aprobar_cobranzas"); // mismo permiso — solo admin/cobranzas
    ...
    useEffect(() => {
        if (!requiereReferencia) formManual.setValue("referencia", "");
    }, [requiereReferencia, formManual]);
    
    useEffect(() => {
        if (idEmpresa) {
            fetchHistorial();
            if (puedeRegistrarManual) fetchClientes();
            if (puedeAprobar) fetchPendientes();
        }
    ...
    const handleClienteChange = async (idCliente: string) => {
        ...
        if (!idCliente) { setFacturasCliente([]); return; }
        try {
    ...
    const onSubmitManual = async (values: z.infer<typeof manualPaymentSchema>) => {
        ...
        try {
            const payload = {
                ...
                metodos: [{ metodo: values.metodo, monto: montoIngresado, referencia: values.referencia, banco: "CAJA OFICINA" }],
    ...
    const getNombreSeguro = (obj: any) => {
        ...
        return obj.nombre_completo || obj.nombre_apellido || obj.nombre || obj.nombres || obj.usuario || obj.username || obj.email || obj.razon_social || 'Sin asignar';
    }
    ...
    const historialFiltrado = historial.filter(c => {
        ...
        const cumpleTexto = nombreCliente.toLowerCase().includes(termino) || nombreVendedor.toLowerCase().includes(termino) || refConsecutivo.toLowerCase().includes(termino);
        let cumpleFecha = true;
        const fechaCobro = c.fecha_reporte.split('T')[0];
        if (filtroFechaInicio && fechaCobro < filtroFechaInicio) cumpleFecha = false;
        if (filtroFechaFin && fechaCobro > filtroFechaFin) cumpleFecha = false;
    ...
    const handleAnular = async (id: string) => {
        if (!window.confirm("¿Está seguro de anular esta cobranza?")) return;
        try {
    ...
    const exportarExcel = () => {
        const data = historialFiltrado.map(h => ({ "Recibo": h.consecutivo, "Fecha": h.fecha_reporte, "Cliente": getNombreSeguro(h.cliente), "Vendedor": getNombreSeguro(h.vendedor || h.cliente?.vendedor), "Monto": Number(h.monto_total) }));
    ...
    const exportarPDF = () => {
        ...
        autoTable(doc, { startY: 20, head: [['Recibo', 'Fecha', 'Cliente', 'Vendedor', 'Monto']], body: historialFiltrado.map(h => [h.consecutivo, new Date(h.fecha_reporte).toLocaleDateString(), getNombreSeguro(h.cliente), getNombreSeguro(h.vendedor || h.cliente?.vendedor), `$${Number(h.monto_total).toFixed(2)}`]) });
        ...
        <div className="flex flex-col h-screen bg-slate-50/30">
            <nav className="h-14 bg-white border-b px-4 flex items-center justify-between shrink-0">
                ...
                <div className="flex gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => { fetchHistorial(); if (puedeAprobar) fetchPendientes(); }} className="h-8">
            ...
            <main className="flex-1 p-6 overflow-y-auto">
                <Tabs defaultValue="historial" className="w-full">
                    <TabsList className="mb-4">
                        {/* Tab 1: visible para todos con ver_cobranzas */}
                        <TabsTrigger value="historial" className="gap-2">
                            <FileText className="h-4 w-4"/> Historial de Cobros
                        </TabsTrigger>

                        {/* Tab 2: solo con aprobar_cobranzas */}
                        {puedeRegistrarManual && (
                            <TabsTrigger value="manual" className="gap-2">
                                <Plus className="h-4 w-4"/> Registro Manual (Caja)
                            </TabsTrigger>
                        )}

                        {/* Tab 3: solo con aprobar_cobranzas */}
                        {puedeAprobar && (
                            <TabsTrigger value="aprobaciones" className="gap-2 relative">
                                <CheckCircle2 className="h-4 w-4"/> Bandeja Aprobaciones
                                {pendientes.length > 0 && (
                                    <span className="ml-1 bg-red-500 text-white text-[9px] px-1.5 rounded-full">{pendientes.length}</span>
                                )}
                            </TabsTrigger>
                        )}
                    </TabsList>
                                    ...
                                    {historialFiltrado.length === 0 ? (
                                        ...
                                        historialFiltrado.map((h) => (
                                            <TableRow key={h.id_cobranza} className="hover:bg-slate-50">
                                                ...
                                                <TableCell className="text-xs font-medium">{getNombreSeguro(h.cliente)}</TableCell>
                                                <TableCell className="text-xs text-slate-500">{getNombreSeguro(h.vendedor || h.cliente?.vendedor)}</TableCell>
                                                <TableCell className="text-xs text-slate-500">
                                                    {Array.isArray(h.metodos) ? h.metodos.map((m: any) => m.metodo).join(', ') : 'Múltiple'}
                                                </TableCell>
                                                ...
                                                <TableCell className="text-center">
                                                    <div className="flex items-center justify-center gap-1">
                                                        ...
                                                        {/* Anular solo para quien puede aprobar */}
                                                        {puedeAprobar && h.estado === 'APLICADA' && (
                    ...
                    {/* TAB 2: REGISTRO MANUAL — solo puedeRegistrarManual */}
                    {puedeRegistrarManual && (
                                                            ...
                                                            {facturasCliente.length === 0 ? (
                                                                ...
                                                                facturasCliente.map(f => (
                                                                    <TableRow key={f.id_factura}>
                                                                        <TableCell>
                                                                            <Checkbox checked={facturasA_Pagar.includes(f.id_factura)} onCheckedChange={() => toggleFactura(f.id_factura, Number(f.saldo_pendiente))} />
                                                                        </TableCell>
                                        ...
                                        <Form {...formManual}>
                                            <form onSubmit={formManual.handleSubmit(onSubmitManual)} className="space-y-4">
                                                ...
                                                <FormField control={formManual.control} name="monto_recibido" render={({ field }) => (
                                                    <FormItem><FormLabel className="text-xs font-bold">Monto Recibido</FormLabel><FormControl><Input type="number" {...field} className="bg-white font-bold" /></FormControl><FormMessage /></FormItem>
                                                )} />
                                                ...
                                                <FormField control={formManual.control} name="referencia" render={({ field }) => (
                                                    <FormItem><FormLabel className="text-xs font-bold">Referencia</FormLabel><FormControl><Input {...field} className="bg-white" disabled={!requiereReferencia} placeholder={requiereReferencia ? "Nro de transferencia/Zelle" : "No aplica para este método"}/></FormControl></FormItem>
                                                )} />
                                                <FormField control={formManual.control} name="nota" render={({ field }) => (
                                                    <FormItem><FormLabel className="text-xs font-bold">Nota Interna</FormLabel><FormControl><Input {...field} className="bg-white" /></FormControl></FormItem>
                                                )} />
                                                <Button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 font-bold mt-4">Registrar Cobranza</Button>
                                            </form>
                                        </Form>
                                    </CardContent>
                                </Card>
                    ...
                    {/* TAB 3: APROBACIONES — solo puedeAprobar */}
                    {puedeAprobar && (
                    ...
                    <DialogContent className="max-w-3xl h-[80vh] flex flex-col p-0 border-none shadow-2xl">
                        <DialogHeader className="p-4 bg-white border-b shrink-0">
                            <DialogTitle className="flex items-center gap-2 text-slate-700">
                                <Eye className="h-5 w-5 text-blue-500" /> Vista Previa del Comprobante
                            </DialogTitle>
                        </DialogHeader>
                        <div className="flex-1 bg-slate-950 flex items-center justify-center p-4 overflow-hidden">
                            {verComprobante && (
                                <img 
                                    ...
                                    alt="Comprobante"
                                    onError={(e) => { (e.target as HTMLImageElement).src = "https://placehold.co/400x600?text=Error+al+cargar+imagen"; }}
                                />
                            )}
                        </div>
                        <DialogFooter className="p-3 bg-slate-50 border-t shrink-0">
                            ...
                            {verComprobante && (
                                <Button asChild>
                                    <a href={verComprobante.startsWith('http') ? verComprobante : `${API_URL}/${verComprobante}`.replace(/\\/g, '/').replace(/([^:]\/)\/{1,}/g, "$1")} target="_blank" rel="noopener noreferrer">
